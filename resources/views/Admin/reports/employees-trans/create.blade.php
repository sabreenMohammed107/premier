...
<ul class="breadcome-menu">
    ...
    <li>
        <span class="bread-blod"> {{ __('titles.empolyee_transactions_reports') }}</span>
    </li>
</ul>
                ...
                <div class="sparkline13-list">
                    <div class="sparkline13-hd">
                        <div class="main-sparkline13-hd">
                            <h1>  {{ __('titles.empolyee_transactions_reports') }} </h1><br />
                        </div>
                    </div>
                    <div class="sparkline13-graph">
                        ...
                        <div class="datatable-dashv1-list custom-datatable-overright">
                            <form action="@if(Auth::user()->role_id == 100 || Auth::user()->role_id == 101 || Auth::user()->role_id == 110){{route('Admin-employee-report.store')}}
                                @else {{route('Company-employee-report.store')}} @endif" method="post" target="_blank">
                                <div class="chosen-select-single mg-b-20">

                                    @csrf
                                    <input type="hidden" name="employee_ids[]" id="deto">
                            ...
                            <table class="table-striped dir-rtl" id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-resizable="true" data-cookie="true" data-cookie-id-table="saveId" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar" >
                                ...
                                <tbody id="indexTable">
                                    @include('Admin.reports.employees-trans.createTable')
...
<script>
    $(document).ready(function() {
        ...
        $('select[name="select_company"]').on('change', function() {
            ...
            $.ajax({
                url: "@if(Auth::user()->role_id == 100 || Auth::user()->role_id == 101 || Auth::user()->role_id == 110){{route('dynamicCompany-employeeReport.fetch')}}@else{{route('Company-dynamicCompany-employeeReport.fetch')}}@endif",
                method: "get",
                data: {
                    company_id: company,
