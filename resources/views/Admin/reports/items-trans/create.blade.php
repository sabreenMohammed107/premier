...
<ul class="breadcome-menu">
    ...
    <li>
        <span class="bread-blod">{{ __('titles.items_transactions_reports') }}</span>
    </li>
</ul>
                ...
                <div class="sparkline13-list">
                    <div class="sparkline13-hd">
                        <div class="main-sparkline13-hd dir-rtl">
                            <h1 > {{ __('titles.items_transactions_reports') }} </h1><br />
                        </div>
                    </div>
                    <div class="sparkline13-graph">
                        ...
                        <div class="datatable-dashv1-list custom-datatable-overright">
                            <form action="@if(Auth::user()->role_id == 100 || Auth::user()->role_id == 101 || Auth::user()->role_id == 110){{route('Admin-item-report.store')}}
                            @else {{route('Company-item-report.store')}}@endif" method="post" target="_blank">
                                <div class="chosen-select-single mg-b-20">

                                    @csrf
                                    <input type="hidden" name="item_ids[]" id="deto">
                            ...
                            <table class="table-striped dir-rtl" id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-resizable="true" data-cookie="true" data-cookie-id-table="saveId" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar" >
                                <thead>
                                    <tr>
                                        ...
                                        <th data-field="name">{{ __('titles.code') }}</th>
                                        <th>{{ __('titles.arabic_name') }}</th>
                                        <th>{{ __('titles.english_name') }}</th>
                                        <th>{{ __('titles.open_balance_items') }}</th>
                                        <th> {{ __('titles.open_blance_one_item') }}</th>
                                    </tr>
                                </thead>
                                <tbody id="indexTable">
                                    @include('Admin.reports.items-trans.createTable')
...
<script>
    $(document).ready(function() {
        ...
        $('select[name="select_company"]').on('change', function() {
            ...
            $.ajax({
                url: "@if(Auth::user()->role_id == 100 || Auth::user()->role_id == 101 || Auth::user()->role_id ==110){{route('dynamicCompany-itemReport.fetch')}}@else{{route('Company-dynamicCompany-itemReport.fetch')}}@endif",
                method: "get",
                data: {
                    company_id: company,
